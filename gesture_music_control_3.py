...
with vidu.PDstream(device, 0) as rgb_stream, vidu.PDstream(device, 1) as tof_stream:
    ...
    previous_index_finger_tip = None
    click_timestamps = []
    action_threshold = 0.2  # 动作阈值，检测移动的距离
    click_duration_threshold = 1.5  # 双击时间间隔阈值（秒）
    z_threshold = 0.2  # z轴阈值，检测快速接近和离开的动作
    ...
    while True:
        ...
        if results.multi_hand_landmarks:
            for hand_landmarks in results.multi_hand_landmarks:
                ...
                # 检测食指指尖的移动方向
                if previous_index_finger_tip is not None and (current_time - last_action_time > action_interval):
                ...
                # 将z轴值添加到列表中
                z_values.append(z)
                if len(z_values) > 4:  # 平滑处理，使用最近5个z轴值的平均值
                    z_values.pop(0)
