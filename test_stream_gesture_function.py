...
# hands = mp_hands.Hands(model_complexity=1, use_gpu=True)
            ...
            for hand_landmarks in results.multi_hand_landmarks:
                for i, landmark in enumerate(hand_landmarks.landmark):
                    ...
                    if i == mp_hands.HandLandmark.INDEX_FINGER_TIP:
                        cv.circle(mat1_resized, (int(x), int(y)), 5, (0, 0, 255), -1)  # 红色
                        cv.circle(mat2, (int(corrected_x), int(corrected_y)), 5, (0, 255, 255), -1)  # 青色
                    else:
                        cv.circle(mat1_resized, (int(x), int(y)), 5, (0, 255, 0), -1)  # 绿色
                ...
                for connection in mp_hands.HAND_CONNECTIONS:
                    ...
                    cv.line(mat1_resized, (int(start_x), int(start_y)), (int(end_x), int(end_y)), (0, 255, 0), 2)
